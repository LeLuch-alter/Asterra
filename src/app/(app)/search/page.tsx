...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      {query && <p className="mt-3 text-sm text-muted-foreground">{total} results</p>}
      ...
      {query && total === 0 && (
        <EmptyState
          ...
          className="mt-8"
          title="Nothing found"
          description="Try a name, an organization, a research field or a project keyword."
          action={
            <Button asChild variant="outline">
              <Link href="/researchers">Browse all researchers</Link>
            </Button>
          }
        />
      )}
      ...
      {people.length > 0 && (
        <section className="mt-10">
          <p className="eyebrow mb-3">People · {people.length}</p>
