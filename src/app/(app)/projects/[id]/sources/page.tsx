...
export default async function SourcesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const targets: SourceTargetOption[] = [
    { value: "project", label: "The project as a whole" },
    { value: "research_question", label: "Research question" },
    { value: "hypothesis", label: "Hypothesis" },
    { value: "methodology", label: "Methodology" },
    ...experiments.map((e) => ({ value: `experiment:${e.id}`, label: `Experiment — ${e.title}`, targetId: e.id })),
    ...results.map((r) => ({ value: `result:${r.id}`, label: `Result — ${r.title}`, targetId: r.id })),
    ...roadmap.map((r) => ({ value: `roadmap_item:${r.id}`, label: `Roadmap step — ${r.title}`, targetId: r.id })),
  ];
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="eyebrow eyebrow-accent">Scientific sources</p>
          <p className="mt-1 text-sm text-muted-foreground">
            Every source says which part of the research it supports, so evidence stays connected to the idea.
          </p>
        </div>
      ...
      {sources.length === 0 ? (
        <EmptyState
          ...
          description={
            isMember
              ? "Attach a paper or dataset to the question, hypothesis, methodology, an experiment or a result."
              : "The team has not attached any sources yet."
          }
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {[s.authors, s.year, formatDate(s.created_at)].filter(Boolean).join(" · ")}
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <Badge variant="outline">
                    {SOURCE_TARGET_LABELS[s.target_type]}
      ...
      <p className="mt-6 text-sm text-muted-foreground">
        ...
        next to the part of the research they support.
      </p>
    </div>
  );
}
