...
└── src/
    ...
    │   ├── page.tsx               # public home: hero + science news feed
    │   ├── globals.css
    │   │
    ...
    │   │   │       ├── graph/page.tsx       # the research graph (interactive map)
    │   │   │       ├── timeline/page.tsx    # how the research evolved + version history
    │   │   │       ├── experiments/page.tsx # experiments and the results they produced
    │   │   │       ├── sources/page.tsx     # scientific sources attached to research parts
...
| Connections, join requests, bookmarks | Server Actions | `src/actions/connections.ts`, `join-requests.ts`, `bookmarks.ts` | small mutations + notifications |
| Avatar upload | Server Action | `src/actions/profile.ts` → Supabase Storage `avatars` | file goes through the server, path is `<user_id>/avatar.<ext>` |
...
connections             (id, requester_id, addressee_id, status: pending|accepted, created_at, responded_at)
project_join_requests   (id, project_id, user_id, message, status: pending|accepted|declined, created_at, responded_at)
bookmarks               (user_id, project_id, created_at)
experiments             (id, project_id, author_id, title, purpose, methodology, data_description,
                         outcome, status, position) -- results link back via research_results.experiment_id
research_sources        (id, project_id, added_by, title, authors, year, url, note,
                         target_type, target_id)    -- a source supports one part of the research
research_versions       (id, project_id, field, version, content, author_id) -- written by triggers
project_activity        (id, project_id, actor_id, type, payload)            -- the research timeline
projects.forked_from    -- research forks; fork_project() copies idea + roadmap into a new project
...
- Notifications: rows are inserted by Server Actions; the browser subscribes to its own rows through Supabase Realtime (`components/layout/notifications-live.tsx`) and shows a toast + `router.refresh()`.
- Gemini provider retries on 429/500/503 and falls back to other models (`lib/ai/providers/gemini.ts`).
- Research graph: `lib/supabase/queries/graph.ts` assembles nodes/edges server-side; `components/graph/research-graph.tsx` lays them out deterministically (idea spine in the middle, people left, sources right, forks above) and renders a pan/zoom SVG — no graph library.
- Timeline and version history are written by database triggers, so they stay correct whichever code path changes a project.
- News: one RSS feed per science field (`lib/news/types.ts`), fetched and cached per category — an article's field is the feed it came from, not a keyword guess.
- Hero/auth scene: CSS 3D (`perspective` + `preserve-3d`) with pointer parallax — real depth without a WebGL dependency.
- Design: flat colours only (no gradients), Instrument Serif for display headings (`.display`), Manrope for body, Geist Mono for tracked labels (`.eyebrow`). Light/dark via `next-themes` (class strategy), tokens in `globals.css`.
